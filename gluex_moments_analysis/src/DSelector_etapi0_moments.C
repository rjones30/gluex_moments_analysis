	...
	if (best_comboCL > 0.01) {
		...
		for (int L=0; L < 13; L += 1) {
			for (int M=-L; M <= L; M += 1) {
				...
				YmomGJ_[momentsGJ] = trial_model::angular_moment(L, M, thetaGJ_, phiGJ_);
				model1moment[momentsGJ] = model1.real_moment(L, M, massEtaPi0, abst);
				model1moment_[momentsGJ] = model1.real_moment(L, M, massEtaPi0_, abst_);
		...
		for (int L=0; L < 13; L += 2) {
			for (int M=-L; M <= L; M += 1) {
				...
				YmomEta_[momentsEta] = trial_model::angular_moment(L, M, thetaEta_, phiEta_);
				++momentsEta;
			}
		}
		...
		for (int L=0; L < 13; L += 2) {
			for (int M=-L; M <= L; M += 1) {
				...
				YmomPi0_[momentsPi0] = trial_model::angular_moment(L, M, thetaPi0_, phiPi0_);
				++momentsPi0;
			}
		}
